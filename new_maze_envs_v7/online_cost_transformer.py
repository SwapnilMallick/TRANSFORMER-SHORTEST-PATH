...
Algorithm 1 (Online Transformer Training Loop) -- modified (Go-Explore reset-replay)
-------------------------------------------------------------------------------------
Implements the mechanism in "modified_approach.pdf" (S. Mallick, UBC #68686534):
instead of always starting each rollout at the true start, every trajectory
resets to the *most epistemically uncertain state already in the graph* and
explores onward from there.

    D <- {}
    for n iterations:
        collect k trajectories with the current transformer; for each one:
            * scan every node s already in the graph: sample
              uncertainty_scan_actions candidate actions from s (read-only
              MC-dropout probe, nothing committed) and take the highest
              resulting std as that state's score
            * s* <- the node with the highest score, provided it clears
              alpha (mu(s*, a) >= alpha for some probed a); otherwise fall
              back to the true start (node 0) -- expected in early
              iterations, when mu is still noisy/uninformative everywhere
            * reset-replay: rebuild the token history along the roadmap's
              known shortest path start -> s* (no sampling, no veto -- this
              path is already validated), then resume ordinary exploration
              from s*
            ...
            * an action reaching an *already-visited* state is VETOED unless
              ...
              cost-to-come for that state, allowing for the model's own
              uncertainty about it.  Rejected actions get a fixed number of
              retries.
...
Uncertainty: epistemic only, via MC dropout
-------------------------------------------
    T stochastic forward passes (dropout left ON) are run over the same
    candidate sequence; their spread gives an epistemic-uncertainty estimate
    for that state:
        c_hat, std_raw   = mean / std of the T passes' last-token predictions
    std_raw (not variance) is used so the estimate stays in cost units. With
    T = mc_samples = 50 the raw std is stable enough to use directly -- there
    is no per-node EMA smoothing. Costs are normalized by the domain diagonal
    (sqrt(2)*L) before training, so c_hat / std_raw and the veto's candidate
    cost are all in normalized units. This same std_raw is reused for two
    distinct purposes:
        1. selecting the reset-replay target s* (mu(s, a) >= alpha), scanning
           uncertainty_scan_actions candidate actions per existing node;
        2. the per-step veto once exploring onward from s*:
               cand_cost / diagonal  <=  c_hat + std_raw
    alpha therefore no longer appears in the veto formula itself -- it is
    purely the threshold used to pick s*.
"""
...
class Config:
    ...
    diagonal: float = math.sqrt(2.0)            # sqrt(L^2 + L^2); set once in main() from L.
                                               # cost-to-come labels & veto costs are divided
                                               # by this so the transformer trains in [0, ~1].
    ...
    alpha: float = 0.05                         # mu(s,a) threshold for picking the
                                               # reset-replay target s* (Go-Explore);
                                               # NOT part of the veto formula anymore
    uncertainty_scan_actions: int = 8           # candidate actions probed per existing
                                               # node when searching for s*
    ...
    mc_samples: int = 50                        # stochastic forward passes per veto query
                                               # (raw std of these passes is used directly
                                               #  in the veto rule; no EMA smoothing)
    ...
    # --- per-iteration uncertainty heatmap ---
    heatmap_res: int = 20                       # grid resolution ([0,L]^2 -> res x res cells)
...
def build_two_corridor_walls(cfg: Config):
    """Two-corridor homotopy maze (start=(0.1,0.56), goal=(0.9,0.56)).

    A solid central island blocks the straight start->goal line, so the goal is
    reachable only by going AROUND it -- over the top or under the bottom. Those
    two routes are distinct homotopy classes. The island is placed high, so:
        * TOP corridor  (over the island) is SHORT  (~1.1)
        * BOTTOM corridor (under the island) is LONG (~1.7)
    This tests whether a planner returns the SHORTER homotopy class."""
    L = cfg.L
    x0, x1 = 0.30, 0.70
    y0, y1 = 0.20, 0.66
    walls = [
...
def build_bug_trap_walls(cfg: Config):
    """Bug trap -- breaks COVERAGE (start=(0.45,0.50), goal=(0.85,0.50)).

    The start sits INSIDE a concave pocket whose solid right wall blocks the
    straight route to the goal; the pocket's only opening is a narrow throat on
    the LEFT (y in [0.42,0.60]), facing AWAY from the goal. To escape, the agent
    must exit left through the throat and wrap around the pocket."""
    L = cfg.L
    x0, x1, y0, y1 = 0.25, 0.65, 0.25, 0.75
    ty0, ty1 = 0.42, 0.60
    walls = [
        ((0, 0), (L, 0)), ((L, 0), (L, L)), ((L, L), (0, L)), ((0, L), (0, 0)),
        ((x0, y1), (x1, y1)),
...
def build_spiral_walls(cfg: Config):
    """Square spiral -- breaks PATH QUALITY (start=(0.07,0.09), goal=(0.50,0.50)).

    The only route from the outer corridor to the centre follows the coil, so the
    optimal path is unavoidably long (~3.6). All corridors >= 0.19 wide (> step)."""
    L = cfg.L
    pts = [(0.00, 0.19), (0.81, 0.19), (0.81, 0.81), (0.19, 0.81),
           (0.19, 0.38), (0.62, 0.38), (0.62, 0.62), (0.38, 0.62)]
    walls = [((0, 0), (L, 0)), ((L, 0), (L, L)), ((L, L), (0, L)), ((0, L), (0, 0))]
    walls += list(zip(pts, pts[1:]))
    return [((np.array(a, float), np.array(b, float))) for a, b in walls]


def build_perfect_maze_walls(cfg: Config, n: int = 5, gen_seed: int = 7):
    """Recursive-backtracker perfect maze -- breaks BOTH. A deterministic
    (fixed gen_seed) n x n maze with a UNIQUE start->goal solution: twisty with
    many dead-ends. Corridors are 1/n = 0.2 wide (> step)."""
    import random
    L = cfg.L
    ...
    for i in range(n):
        for j in range(n):
            if i + 1 < n and frozenset({(i, j), (i + 1, j)}) not in carved:
                x = (i + 1) / n
                walls.append(((x, j / n), (x, (j + 1) / n)))
            if j + 1 < n and frozenset({(i, j), (i, j + 1)}) not in carved:
                y = (j + 1) / n
                walls.append(((i / n, y), ((i + 1) / n, y)))
    return [((np.array(a, float), np.array(b, float))) for a, b in walls]


# env name -> spec. Grid envs give {"grid": (cols, rows)}; custom envs give a
# {"builder": fn}. Every entry fixes the start/goal for that geometry; custom
# envs may add a "label" for display.
ENVS = {
    "four_room":    {"grid": (2, 2), "start": (0.10, 0.10), "goal": (0.90, 0.90)},
    "eight_room":   {"grid": (4, 2), "start": (0.10, 0.10), "goal": (0.90, 0.90)},
    "decoy":        {"builder": build_decoy_walls, "label": "bait-chamber decoy",
                     "start": (0.10, 0.50), "goal": (0.90, 0.50)},
    "two_corridor": {"builder": build_two_corridor_walls,
                     "label": "two-corridor homotopy",
                     "start": (0.10, 0.56), "goal": (0.90, 0.56)},
    "bug_trap":     {"builder": build_bug_trap_walls, "label": "bug trap",
                     "start": (0.45, 0.50), "goal": (0.85, 0.50)},
    "spiral":       {"builder": build_spiral_walls, "label": "square spiral",
                     "start": (0.07, 0.09), "goal": (0.50, 0.50)},
    "perfect_maze": {"builder": build_perfect_maze_walls,
                     "label": "recursive-backtracker maze",
...
def collect_trajectory(model, graph, cfg, walls, rng,
                       start_node: int = 0,
    ...
    tokens[t] = fuse(s_t, a_t); node_ids[t] = merged-graph id of the resulting
    state s_{t+1}.

    Rollouts no longer always start at cfg.start: `start_node`/`start_tokens`/
    `start_node_ids`/`start_path_cost` are the reset-replay target s* and its
    roadmap history (see find_uncertain_state), reconstructed with no new
    sampling or veto since that path is already validated. Passing the
    defaults (start_node=0, no history, zero path cost) reproduces the
    original start-anchored rollout.

    The veto uses the raw MC-dropout std (std_raw from predict_last_mc) directly
    as the epistemic-uncertainty term -- no per-node EMA is kept.

    Retry-failure diagnostics (one of three ways a retry can be lost):
      n_sample_fail   -- sample_free exhausted max_sample_tries with no
                         in-bounds, collision-free candidate at all (pure
                         geometry; never reaches the merge/veto logic).
      n_wall_fallback -- the nearest-node merge target was wall-blocked, so
                         the candidate was added as a new node instead of
                         merging into it (no retry lost -- counts how often
                         this fallback fires).
      n_veto          -- a valid, wall-safe merge candidate was rejected by
                         the cost/uncertainty veto."""
    goal = np.array(cfg.goal, float)
    s = np.array(graph.pos[start_node], float)
    s_node = start_node
    ...
    for _ in range(cfg.max_steps):
        ...
        for _retry in range(cfg.max_retries):
            ...
            if visited:
                ...
                # Re-check the actual edge.
                node_pos = graph.pos[j]
                if collides(s, node_pos, walls):
                    # The nearest existing node is close in straight-line
                    # distance but on the wrong side of a wall (e.g. the next
                    # coil over in a spiral corridor) -- merging into it would
                    # silently create an edge that tunnels through the wall.
                    # s_next itself was already validated collision-free by
                    # sample_free (s -> s_next, not s -> node_pos), so instead
                    # of discarding this candidate and burning a retry over an
                    # invalid merge, fall back to treating it as a brand-new,
                    # unmerged node -- the same unconditional-add path used
                    # below for any other first-time state, no veto involved.
                    n_wall_fallback += 1
                    ...
                    cand_cost = path_cost + step_cost
                    # c_hat / std_raw are in NORMALIZED cost units (labels were
                    # divided by cfg.diagonal at train time), so put the raw
                    # candidate cost on the same scale for the veto comparison.
                    cand_cost_n = cand_cost / cfg.diagonal
                    c_hat, std_raw = predict_last_mc(model, tokens + [tok], cfg)
                    # take the action iff current path is competitive with
                    # best-known; raw MC-dropout std is the epistemic-
                    # uncertainty term (no EMA)
    ...
    # embedding table's max_len rows; keep the trailing window, matching the
    # truncation predict_last_mc already applies at veto time.
...
# --------------------------------------------------------------------------- #
# Go-Explore reset-replay: pick the most uncertain existing state, s*
# --------------------------------------------------------------------------- #
def build_all_histories(graph: Graph, g: List[float], prev: List[int]):
    """Token/node-id history (start -> node) for every node in the roadmap's
    Dijkstra tree, built bottom-up from `prev` in O(n_nodes). Actions are
    reconstructed as the position delta between consecutive path nodes (the
    original noise term isn't recoverable, but the displacement is the same
    quantity fuse() stores as `a` elsewhere). Unreachable nodes get an empty
    history (shouldn't occur -- every roadmap node was added via a real,
    connected transition -- but the check is cheap)."""
    n = len(graph.pos)
    order = sorted(range(n), key=lambda i: g[i])
    tok_hist: List[List[np.ndarray]] = [[] for _ in range(n)]
    id_hist: List[List[int]] = [[] for _ in range(n)]
    for node in order:
        if node == 0 or not math.isfinite(g[node]):
            continue
        p = prev[node]
        a = graph.pos[node] - graph.pos[p]
        tok_hist[node] = tok_hist[p] + [fuse(graph.pos[p], a)]
        id_hist[node] = id_hist[p] + [node]
    return tok_hist, id_hist


def find_uncertain_state(model, graph: Graph, g: List[float], prev: List[int],
                         cfg: Config, walls, rng,
                         cache: Dict[int, Tuple[float, float]]):
    """Go-Explore-style selection of the reset-replay target s*.

    For every node currently in the graph, sample cfg.uncertainty_scan_actions
    collision-free candidate actions (same generator as sample_free) and query
    the model's MC-dropout std for each -- a read-only probe: nothing is added
    to the graph or committed. s* is the node whose best (highest-std)
    candidate has the largest std_raw overall.

    `cache` memoizes each node's best-candidate score across the k trajectories
    collected within a single iteration: the model's weights are fixed for the
    whole iteration (training only runs once, after all k trajectories are
    collected), so a node's true uncertainty landscape cannot change within
    it -- only the set of nodes (new ones added by earlier trajectories this
    iteration) and, occasionally, an existing node's replay history (if a
    shorter path to it was just discovered) can. A cached score is reused as
    long as the node's g[node] hasn't changed since it was cached; otherwise
    (or if the node has never been scored this iteration) it's rescanned and
    the cache entry updated. The caller must pass a fresh, empty `cache` at
    the start of every iteration -- a cache is only ever valid for the model
    that produced it.

    On a cache hit, `sample_free` is still called cfg.uncertainty_scan_actions
    times (drawing and discarding candidates) so this scan consumes exactly as
    many draws from `rng` as an uncached scan would -- `rng` is shared with the
    real exploration in collect_trajectory, so skipping those draws on a hit
    would desync it and change which states get explored, not just how fast
    the scan runs. Only the expensive part, the predict_last_mc forward pass,
    is actually skipped on a hit. (PyTorch's own dropout RNG, consumed inside
    predict_last_mc, is a separate stream and isn't preserved this way -- a
    cache hit does still shift *that* stream slightly relative to an uncached
    run, so MC-dropout std estimates elsewhere in the run won't be perfectly
    bit-identical to a no-cache run, only the sampled states/actions will be.)

    Falls back to the true start (node 0) if no candidate anywhere clears
    cfg.alpha -- expected in early iterations, when the model is still
    untrained and/or the graph only covers a handful of near-start states, so
    mu carries little real signal yet.

    Returns (s_star, tokens_to_s_star, node_ids_to_s_star, path_cost_to_s_star,
    ...
    for node in range(len(graph.pos)):
        ...
        for _ in range(cfg.uncertainty_scan_actions):
            # always drawn, hit or miss, to keep rng's position (and hence the
            # real exploration downstream) independent of caching
            cand = sample_free(s, goal, cfg, walls, rng)
            if is_hit or cand is None:
                continue
            a, _s_next = cand
            _, std_raw = predict_last_mc(model, tok_hist[node] + [fuse(s, a)], cfg)
...
def build_sequences(D, g, diagonal: float = 1.0):
    """Turn the replay buffer into (tokens, dijkstra-cost targets) sequences.

    Targets are the Dijkstra cost-to-come g[node] divided by `diagonal` (the
    domain diagonal sqrt(2)*L), so the transformer trains in normalized cost
    units rather than raw distance."""
    seqs = []
    for tokens, node_ids, _ in D:
        if not node_ids:
            continue
        tgts = np.array([g[i] for i in node_ids], float) / diagonal
...
def train_transformer(model, D, g, cfg, rng):
    seqs = build_sequences(D, g, cfg.diagonal)
    if not seqs:
        return float("nan")
...
def field_mae(model, D, g, cfg):
    ...
    model.eval()
    seqs = build_sequences(D, g, cfg.diagonal)      # normalized-space error
    if not seqs:
        return float("nan")
    err, n = 0.0, 0
...
# --------------------------------------------------------------------------- #
# Per-iteration cell-based uncertainty heatmap
# --------------------------------------------------------------------------- #
def save_uncertainty_heatmap(model, graph, cfg: Config, walls, it: int, log=None):
    """Spatial map of the transformer's MC-dropout uncertainty after this
    iteration's training.

    The [0, L]^2 domain is split into a heatmap_res x heatmap_res grid. Every
    roadmap node is assigned to its cell and its MC-dropout std recorded
    (predict_last_mc on the history-free zero-action token [x, y, 0, 0]); a
    cell's value is the mean std of the nodes that fall in it. Cells with no
    node are "unknown" and drawn at max uncertainty. Colormap 'YlGnBu_r':
    yellow = high uncertainty (unexplored / poorly known), dark blue = low
    (well explored, confident). Maze walls, start and goal are overlaid.

    Saved as {env}_uncertainty_iter{it:02d}.png -- an *extra* per-iteration
    output, alongside (not replacing) the final {env}_cost.gif."""
    try:
        import matplotlib
        matplotlib.use("Agg")
        import matplotlib.pyplot as plt
    ...
    res, L = cfg.heatmap_res, cfg.L
    sum_std = np.zeros((res, res), dtype=np.float64)
    cnt = np.zeros((res, res), dtype=np.int64)
    for p in graph.pos:
        tok = np.array([p[0], p[1], 0.0, 0.0], dtype=np.float32)   # zero-action token
        _, std_raw = predict_last_mc(model, [tok], cfg)
        col = min(int(p[0] / L * res), res - 1)                    # x -> column
        row = min(int(p[1] / L * res), res - 1)                    # y -> row
        sum_std[row, col] += std_raw
        cnt[row, col] += 1

    grid = np.full((res, res), np.nan, dtype=np.float64)
    nz = cnt > 0
    grid[nz] = sum_std[nz] / cnt[nz]
    finite = grid[np.isfinite(grid)]
    vmin = float(finite.min()) if finite.size else 0.0
    vmax = float(finite.max()) if finite.size else 1.0
    if vmax <= vmin:
        vmax = vmin + 1e-6
    grid_filled = np.where(np.isfinite(grid), grid, vmax)          # unknown cells -> max uncertainty

    fig, ax = plt.subplots(figsize=(6, 6))
    im = ax.imshow(grid_filled, origin="lower", extent=[0, L, 0, L],
                   cmap="YlGnBu_r", vmin=vmin, vmax=vmax, aspect="equal",
                   interpolation="nearest", zorder=1)
    fig.colorbar(im, ax=ax, label="MC-dropout std  (epistemic uncertainty)")

    for (w0, w1) in walls:
        ax.plot([w0[0], w1[0]], [w0[1], w1[1]], color="k", lw=2, zorder=3)
    ax.scatter(*cfg.start, c="white", edgecolors="k", s=90, zorder=4, label="start")
    ax.scatter(*cfg.goal, marker="*", c="gold", edgecolors="k", s=240, zorder=4,
               label="goal")
    ax.set_xlim(-0.02, L + 0.02)
    ax.set_ylim(-0.02, L + 0.02)
    ax.set_aspect("equal")
    ax.set_title(f"{cfg.env} ({env_label(cfg)}): uncertainty heatmap  iter {it}")
    ax.legend(loc="lower right", fontsize=8)
    fig.tight_layout()

    out = os.path.join(cfg.outdir, f"{cfg.env}_uncertainty_iter{it:02d}.png")
...
def main(cfg: Config = Config()):
    ...
    # domain diagonal: computed once here, reused for label + veto normalization
    cfg.diagonal = math.sqrt(2.0) * cfg.L
    ...
    log(f"cost normalization: labels & veto costs divided by diagonal = "
        f"sqrt(2)*L = {cfg.diagonal:.4f}  (loss & fieldMAE are in normalized "
        f"cost units; reported path costs stay in raw distance units)")
    ...
    log(f"veto: c <= c_hat + mu (raw MC-dropout std, no EMA)  |  mc_samples = "
        f"{cfg.mc_samples}")
    log(f"[v7] per-iteration uncertainty-scan cache: a node's score is reused "
        f"across the k trajectories of one iteration as long as its g[node] "
        f"hasn't changed; the cache is reset every iteration (training just "
        f"changed the model, so all cached scores go stale)")
    ...
    for it in range(cfg.n_iters):
        ...
        # Each trajectory independently re-scans the (growing) graph for its
        # own reset-replay target s* -- see find_uncertain_state / Algorithm 1
        # in the module docstring. uncertainty_cache memoizes per-node scores
        # across the k trajectories of THIS iteration only (the model is fixed
        # until training runs at the end of the iteration); it's reset fresh
        # every iteration since training invalidates every cached score.
        succ, vetoes, steps = 0, 0, 0
        sample_fails, wall_fallbacks = 0, 0    # retry-failure diagnostics
        resets_to_start = 0                    # how many of the k picked s*=0
        ...
        for _ in range(cfg.k_trajectories):
            ...
            s_star, tok_hist, id_hist, cost_hist, n_hits, n_miss = find_uncertain_state(
                model, graph, g_cur, prev_cur, cfg, walls, rng, uncertainty_cache)
            resets_to_start += int(s_star == 0)
            cache_hits += n_hits
            cache_misses += n_miss
            tokens, node_ids, reached, nv, nsf, nwf = collect_trajectory(
                model, graph, cfg, walls, rng,
        ...
        # per-iteration spatial uncertainty map of the *just-trained* model
        save_uncertainty_heatmap(model, graph, cfg, walls, it, log)
    ...
    log(f"Field cost-to-come MAE (transformer vs Dijkstra, normalized units): "
