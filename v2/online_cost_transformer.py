...
-----------------------------------------
    ...
    * 2 x nn.TransformerEncoderLayer, d_model=64, nhead=2, GELU
    * causal mask (upper-triangular -inf above the diagonal)
    * two linear heads: cost c_hat and uncertainty mu (softplus > 0)
    * Huberised Gaussian NLL:  L = 0.5 log(mu) + Huber(c - c_hat) / mu
"""
...
class Config:
    ...
    explore_iters: int = 0                       # first N iters collect veto-free (warm-up)
    max_steps: int = 60
    max_retries: int = 6                        # veto retries per step
    max_sample_tries: int = 25                  # collision-free resamples
    alpha: float = 0.05                         # uncertainty discount in veto
...
class CostTransformer(nn.Module):
    def __init__(self, cfg: Config):
        ...
        self.unc_head = nn.Linear(cfg.d_model, 1)
    ...
    def forward(self, x, attn_mask, pad_mask=None):
        ...
        c = self.cost_head(h).squeeze(-1)                     # (B, T)
        mu = F.softplus(self.unc_head(h)).squeeze(-1) + 1e-3  # (B, T), > 0
        return c, mu


@torch.no_grad()
def predict_last(model: CostTransformer, tokens: List[np.ndarray], cfg: Config):
    """c_hat, mu for the *last* token of a single sequence (used at veto time)."""
    model.eval()
    toks = tokens[-cfg.max_len:]
    x = torch.tensor(np.array(toks)[None], dtype=torch.float32, device=cfg.device)
    c, mu = model(x, causal_mask(x.shape[1], cfg.device), None)
    return float(c[0, -1]), float(mu[0, -1])


# --------------------------------------------------------------------------- #
# Huberised Gaussian NLL
# --------------------------------------------------------------------------- #
def huber_nll(c, mu, tgt, pad, cfg: Config):
    valid = ~pad
    err = (tgt - c)[valid]
    mu_v = mu[valid]
    ae = err.abs()
    ...
    loss = 0.5 * torch.log(mu_v) + huber / mu_v
    return loss.mean()
...
def collect_trajectory(model, graph, cfg, walls, rng, use_veto=True):
    """Returns (tokens, node_ids, reached, n_veto).  tokens[t] = fuse(s_t, a_t);
    node_ids[t] = merged-graph id of the resulting state s_{t+1}.

    When use_veto is False (exploration warm-up), any collision-free step is
    accepted -- visited or not -- so trajectories traverse freely and the
    frontier can advance; the cost-to-come veto is skipped entirely."""
    ...
    for _ in range(cfg.max_steps):
        ...
        for _retry in range(cfg.max_retries):
            ...
            if visited:
                # merge into the existing node. The sampler checked s -> s_next,
                # but the STORED edge is s -> pos[j] (the merged node), which can
                # differ by up to novelty_radius and cross a wall the sampled
                # segment did not. Re-check the actual edge and reject if so,
                # otherwise merging can create wall-crossing shortcut edges that
                # Dijkstra/greedy then prefer.
                ...
                if use_veto:
                    c_hat, mu = predict_last(model, tokens + [tok], cfg)
                    # take the action iff current path is competitive with best-known
                    if not (cand_cost < c_hat + (mu - cfg.alpha)):
                        n_veto += 1
                        continue                               # VETO -> retry
...
def train_transformer(model, D, g, cfg, rng):
    ...
    for _ in range(cfg.train_epochs):
        ...
        for i in range(0, len(order), cfg.batch_size):
            ...
            c, mu = model(x, causal_mask(x.shape[1], cfg.device), pad)
            loss = huber_nll(c, mu, tgt, pad, cfg)
            opt.zero_grad()
            loss.backward()
...
def field_mae(model, D, g, cfg):
    ...
    for i in range(0, len(seqs), cfg.batch_size):
        ...
        c, _ = model(x, causal_mask(x.shape[1], cfg.device), pad)
        v = ~pad
...
def predict_cost_field(model, graph, cfg):
    ...
    for i in range(0, len(P), bs):
        ...
        c, _ = model(x, causal_mask(1, cfg.device), None)
...
def main(cfg: Config = Config()):
    ...
    log(f"horizon (max_steps) = {cfg.max_steps}  |  pos-emb rows (max_len) = "
        ...
        f"|  goal_bias = {cfg.goal_bias}  |  explore_iters = {cfg.explore_iters}")
    ...
    for it in range(cfg.n_iters):
        ...
        # The first `explore_iters` iterations run veto-free (warm-up) so the
        # roadmap can cover the maze before the veto engages.
        use_veto = it >= cfg.explore_iters
        succ, vetoes, steps = 0, 0, 0
        for _ in range(cfg.k_trajectories):
            tokens, node_ids, reached, nv = collect_trajectory(
                model, graph, cfg, walls, rng, use_veto=use_veto)
...
def parse_args():
    ...
    p.add_argument("--explore-iters", type=int, default=None, dest="explore_iters",
                   help="first N iterations collect veto-free (exploration "
                        "warm-up) before the veto engages (default: 0)")
...
if __name__ == "__main__":
    ...
    if args.explore_iters is not None:
        cfg.explore_iters = args.explore_iters
