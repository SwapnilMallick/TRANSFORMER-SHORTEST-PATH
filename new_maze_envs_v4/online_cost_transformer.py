...
-------------------------------------------
    ...
    cost are all in normalized units.
    veto rule (fixed alpha):
        cand_cost / diagonal  <=  c_hat + (std_raw - alpha)
"""
        ...
        for _retry in range(cfg.max_retries):
            ...
            if visited:
                ...
                if collides(s, node_pos, walls):
                    ...
                    if cand_cost_n <= c_hat + (std_raw - cfg.alpha):
...
def main(cfg: Config = Config()):
    ...
    log(f"veto uncertainty: raw MC-dropout std (no EMA)  |  mc_samples = "
        f"{cfg.mc_samples}")
    hdr = (f"{'iter':>4} {'nodes':>6} {'trans':>7} {'succ':>5} {'len':>5} "
           f"{'veto':>5} {'sfail':>6} {'wallfb':>7} {'loss':>8} {'fieldMAE':>9}")
    log(hdr)
    for it in range(cfg.n_iters):
        ...
        log(f"{it:>4} {len(graph.pos):>6} {n_trans:>7} "
            ...
            f"{sample_fails:>6} {wall_fallbacks:>7} "
