...
class RobosuiteTask:
    def __init__(self, cfg: TaskConfig = TaskConfig(), seed: int = None,
        ...
        enable_render: build the REAL env (phase 2) with offscreen rendering so
        task.ik.render(camera, h, w) works -- required by the image-state
        experiment, which embeds rendered views of every node. Only phase 2 is
        affected: the phase-1 probe and verify_cube_frozen's fresh envs stay
        non-rendering (they never render)."""
        self.cfg = cfg
        ...
        self.ik = RobosuiteIK(
            ...
            seed=seed, enable_render=enable_render)
