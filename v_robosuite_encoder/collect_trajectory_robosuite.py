...
Image-state version (this folder)
---------------------------------
Every transformer token is now  [ node_emb[s_t].flatten() , a_t ]  -- the
standardized DINOv2 embedding of V camera views of the arm at the CURRENT
node, then the raw 3-D action -- instead of [x, y, z, a]. `node_emb` maps node
id -> (V, D) embedding and is a sibling of `node_q`: each node's embedding is
rendered ONCE, when the node is created, at that node's stored joint
configuration. Geometry is untouched: `s` (EEF xyz) still drives step costs,
merging, the veto's comparison cost and task.reached; it never enters a token.

  * A NEW node is embedded right after the explicit re-sync
    world.ik.set_q(q_node) at commit time, never earlier: in the collision
    fallback branch world.collides may have returned early and left the sim at
    an intermediate substep, so an earlier render would show the wrong pose.
  * A MERGED node reuses node_emb[j] (rendered earlier at ITS stored q); it is
    never re-rendered, so the embedding always matches node_q[j].
  * The uncertainty scan renders nothing: a probed token needs the embedding of
    the CURRENT node (already cached) plus the candidate action, and the
    candidate's resulting state is never committed.
...
def state_feat(node_emb: Dict[int, np.ndarray], n: int) -> np.ndarray:
    """Flat (V*D,) state half of a token for node `n`."""
    return node_emb[n].reshape(-1)


def collect_trajectory_robosuite(
    model, graph: Graph, cfg: Config, task: RobosuiteTask, rng,
    node_q: Dict[int, np.ndarray],
    node_emb: Dict[int, np.ndarray], encoder,
    start_node: int = 0,
    ...
    node_emb: node id -> (V, D) standardized image embedding, mutated in place
    (one entry per node, like node_q); `encoder` is the calibrated
    image_encoder.ImageEncoder used to embed every NEW node. Tokens are
    [node_emb[s_node].flatten(), action]; see the module docstring.
    ...
    assert 0 in node_emb, "node 0 must be embedded by the caller before collecting"
    tok_width = encoder.n_views * encoder.emb_dim + 3
    if start_node != 0:
    ...
    for _ in range(cfg.max_steps):
        ...
        for _retry in range(cfg.max_retries):
            ...
            a, s_next, q_to = cand
            tok = fuse(state_feat(node_emb, s_node), a)   # image state + raw action; xyz never enters
            assert tok.shape[0] == tok_width, (
                f"token width {tok.shape[0]} != {tok_width} (a stray xyz token?)")
            new_node = False
            ...
            if new_node:
                # embed only AFTER the re-sync above: in the collision-fallback
                # branch world.collides may have left the sim at an
                # intermediate substep, so any earlier render would show the
                # wrong pose. Merged nodes reuse the embedding rendered when
                # they were created.
                node_emb[node_id] = encoder.embed(task.ik)
            committed = True
            break
...
def find_uncertain_state_robosuite(
    ...
    Image-state tokens: the history comes from build_all_histories with the
    embedding as the state feature, and each probe token is
    [node_emb[node].flatten(), a]. Nothing is rendered here -- a probe only
    needs the CURRENT node's cached embedding (the candidate's resulting
    ...
    """
    world = task.world
    tok_hist, id_hist = build_all_histories(
        graph, g, prev, state_feat=lambda n: state_feat(node_emb, n))
    tok_width = node_emb[0].size + 3
    ...
    for node in range(len(graph.pos)):
        ...
        if is_hit:
            ...
            for _ in range(cfg.uncertainty_scan_actions):
                ...
                tok = fuse(feat, a)
                assert tok.shape[0] == tok_width, (
                    f"probe token width {tok.shape[0]} != {tok_width}")
                _, std_raw = predict_last_mc(model, tok_hist[node] + [tok], cfg)
...
if __name__ == "__main__":
    from online_cost_transformer import ImageCostTransformer
    ...
    task = RobosuiteTask(seed=0, enable_render=True)
    cfg.diagonal = world_diagonal(task.world.cfg)
    enc_cfg = EncoderConfig(n_calib=40)
    encoder = ImageEncoder(enc_cfg)
    encoder.calibrate(task, np.random.default_rng(2))
    task.reset_to_start()
    node_emb: Dict[int, np.ndarray] = {0: encoder.embed(task.ik)}
    ...
    print(f"diagonal = {cfg.diagonal:.4f} | token width = "
          f"{encoder.n_views * encoder.emb_dim + 3}")
    ...
    if node_ids:
        ...
        synced = bool(np.allclose(sim_eef, stored_pos, atol=1e-3))
        print("match:", synced)
        ok_all &= synced
    q_ok = len(node_q) == len(graph.pos)
    emb_ok = len(node_emb) == len(graph.pos) == len(node_q)
    print("node_q entries == graph nodes:", q_ok)
    print("node_emb entries == graph nodes == node_q entries:", emb_ok)
    ok_all &= q_ok and emb_ok

    # embeddings must be tied to each node's STORED config: teleport to a few
    # random nodes and re-embed -- the result must match what was stored when
    # the node was created (this moves the sim, so it runs after the sync check)
    pick = np.random.default_rng(3).choice(len(graph.pos), size=min(3, len(graph.pos)),
                                           replace=False)
    for n in pick:
        task.ik.set_q(node_q[int(n)])
        fresh = encoder.embed(task.ik)
        same = bool(np.allclose(fresh, node_emb[int(n)], atol=1e-4))
        print(f"embedding consistency, node {int(n):4d}: "
              f"max|diff|={np.abs(fresh - node_emb[int(n)]).max():.2e} -> {same}")
        ok_all &= same
    ...
    if len(graph.pos) > 1:
        ...
        s_star, tok_hist, id_hist, cost_hist, n_hits, n_miss = find_uncertain_state_robosuite(
            model, graph, g, prev, cfg, task, scan_rng, node_q, node_emb, cache)
        ...
        landed = bool(np.allclose(eef_after_replay, graph.pos[s_star], atol=1e-6))
        print("reset-replay lands on s*:", landed)
        ok_all &= landed

    print("\nALL SMOKE CHECKS PASSED" if ok_all else "\nSOME SMOKE CHECKS FAILED")
    task.close()
