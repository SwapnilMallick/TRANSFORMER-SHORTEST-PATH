...
def main():
    ...
    p.add_argument("--env", required=True, choices=list(ENV_INFO),
                   help="maze environment / results directory (e.g. 4room, 8room, decoy)")
    p.add_argument("--beta", default=None,
                   help="mc_ema_beta value (e.g. 0.2); when given, the alpha_* dirs "
                        "are read from {env}/beta_{beta}/ instead of {env}/")
    p.add_argument("--out-dir", required=True,
                   help="directory to save the plots into; created automatically if missing")
    args = p.parse_args()

    prefix = ENV_INFO[args.env]
    base = args.env if args.beta is None else os.path.join(args.env, f"beta_{args.beta}")
    subs = sorted(glob.glob(os.path.join(base, "alpha_*")))
    if not subs:
        raise SystemExit(f"no alpha_* subdirectories found under {base}/")
