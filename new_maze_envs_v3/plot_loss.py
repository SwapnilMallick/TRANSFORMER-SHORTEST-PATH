"""
Plot Huber training loss vs. training iteration for the whole new_maze_envs_v3
alpha sweep: a single image with one subplot per maze environment (4room,
8room, decoy, bug_trap, perfect_maze, spiral), each holding one colored curve
per alpha value (alpha_0.00 .. alpha_0.10). The same alpha gets the same color
in every subplot.

Loss values are read from {env}/alpha_*/{prefix}_metrics.csv (the "loss"
column -- identical to the per-iteration loss printed in {prefix}_run.log).
The v3 sweep has no mc_ema_beta dimension, so there is a single flat set of
alpha_* runs per environment. Output: plots_huber_loss/loss_all_envs.png

Usage (run from new_maze_envs_v3/):
    python plot_loss.py
    python plot_loss.py --out-dir plots_huber_loss
"""
...
def main():
    ...
    p.add_argument("--out", default=None,
                   help="explicit output path; overrides --out-dir/loss_all_envs.png")
    args = p.parse_args()

    # load every environment's alpha runs up front
    per_env = {}
    for env, prefix in ENV_INFO.items():
        per_env[env] = load_loss(os.path.join(args.root, env), prefix)
    ...
    fig.suptitle("Huber training loss vs. training iteration, by alpha -- "
                 "per environment (new_maze_envs_v3)")
    fig.tight_layout(rect=(0, 0.06, 1, 0.96))

    out = args.out or os.path.join(args.out_dir, "loss_all_envs.png")
