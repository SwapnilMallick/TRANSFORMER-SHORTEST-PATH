"""
Plot field cost-to-come MAE vs. training iteration, one subplot per maze
environment, one colored line per alpha value in that environment's sweep.

Reads {env_dir}/alpha_*/{prefix}_metrics.csv (iter, field_mae columns) from
the existing online_cost_transformer.py --alpha-sweep results and saves a
single combined image with three side-by-side subplots (4room, 8room, decoy).
The same alpha value gets the same color across all three subplots.

Usage (run from v3/, where 4room/, 8room/, decoy/ already exist):
    python plot_field_mae.py [--out field_mae_by_alpha.png]
"""
...
def load_field_mae(env_dir: str, prefix: str):
    """Returns {alpha_str: (iters, field_mae_values)}, sorted by alpha."""
    runs = {}
...
def main():
    ...
    p.add_argument("--out", default="field_mae_by_alpha.png",
                   help="output image path (default: field_mae_by_alpha.png)")
    args = p.parse_args()
    ...
    for ax, (env_dir, prefix, title) in zip(axes, ENV_DIRS):
        runs = load_field_mae(env_dir, prefix)
        if not runs:
