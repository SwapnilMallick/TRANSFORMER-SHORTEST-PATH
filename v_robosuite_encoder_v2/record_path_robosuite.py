...
reused completely unchanged: it only consumes the predicted-cost array and the
geometric graph; the image-state model is queried with each node's cached
embedding, see extract_transformer_path_robosuite) as a side-by-side
...
from collect_trajectory_robosuite import state_feat
...
def extract_transformer_path_robosuite(model, graph: Graph, cfg, task: RobosuiteTask,
    ...
    The image-state model is queried with each node's cached embedding (plus a
    zero action) rather than its position, so predict_cost_field is given
    state_feats; transformer_path / path_length need no adaptation (they only
    consume the resulting cost array and the geometric graph).
    ...
    feats = np.stack([state_feat(node_emb, i) for i in range(len(graph.pos))])
    chat = predict_cost_field(model, graph, cfg, state_feats=feats, action_dim=3)
