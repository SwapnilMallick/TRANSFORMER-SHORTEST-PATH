...
def main(cfg: Config, outdir: str = "robosuite_encoder_results", record_gif: bool = True,
         enc_cfg: EncoderConfig = None, token_fusion: str = "concat"):
    ...
    model = ImageCostTransformer(
        ...
        view_dropout=enc_cfg.view_dropout,
        token_fusion=token_fusion).to(cfg.device)
    ...
    enc_width = model.cost_head.in_features
    log(f"token fusion = {token_fusion}  |  encoder width = {enc_width}  |  "
        f"dim_ff = {cfg.dim_ff * (1 if token_fusion == 'sum' else 2)}  |  "
        f"transformer params = {n_params:,}")
...
def parse_args():
    ...
    p.add_argument("--view-dropout", type=float, default=0.0, dest="view_dropout",
                   help="probability of dropping a whole view per token, training "
                        "only (default: 0)")
    p.add_argument("--token-fusion", default="concat", choices=["concat", "sum", "wide_sum"],
                   dest="token_fusion",
                   help="concat = [state ; action] (2*d_model wide) + learned position "
                        "embedding, encoder width 128 (default); sum = state + action + "
                        "position at d_model 64 (as in v_robosuite_encoder); wide_sum = "
                        "the same sum with every width doubled (128), the control for concat")
